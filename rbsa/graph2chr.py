...
#from falcon_kit.io import open
...
def build_sg(sg_edges_list_fn, paf_fn, ctg_paths_fn):
    ...
    with open(paf_fn) as f:
        for line in f:
            ...
            g_s = 0 if l[4] == '+' else 1
            # contained_etc = l[12]
            # f_strand, f_start, f_end, f_len = 0, l[7], l[8], l[6]
            # g_start, g_end, g_len = l[2], l[3], l[1]
...
def shortestpath(sg,ctg_id,start,t,edges,a_ctg_group):
    all_alt_path = []
    c_graph = nx.ego_graph(sg, start, radius=200)
    ...
    print("gap",ctg_id,start,t,shortest_path)
...
def traverse_sg(ctg, sg):
    ...
    a_ctg_group = {}
    edges = {}
    #utg_data_new = open("utg_data_new","w")
    for key in ctg.keys():
        l = ctg[key]
        ...
        while i < len(utgs):
            ...
            if v != "gap":
                ...
                # print(s,v,t)
                n_ego_graph = nx.ego_graph(sg, s, radius=50)
                # n_ego_graph = sg
                ...
                if len(s_path) != 0:
                    gap_path = shortestpath(n_ego_graph,ctg_id, s, t, edges, a_ctg_group)
                    ...
                    j = 0
                    rd = 30
                    while k <= 50 and len(tmp_utg_path) > 0 and len(s_path) == 0 :
                        j = 1
                        m = utgs[i + 1].split("~")
                        while len(s_path) == 0 and i  + 1 < len(utgs) and j <=50 and j < len(m):
                            t = m[j]
                            print("lk",ctg_id,start,t)
                            ...
                            if t  in n_ego_graph.nodes():
                                try:
                                    ...
                                    s_path = []
                                    #print("except",strat,t)
                            if len(s_path) != 0:
                                print(ctg_id, start, t)
                                gap_path = shortestpath(n_ego_graph,ctg_id, start, t, edges, a_ctg_group)
...
def run(improper_p_ctg, proper_a_ctg, preads_fasta_fn, sg_edges_list_fn,paf_fn, ctg_paths_fn):
    print("phase 1")
    ...
    seqs = {}
    with open(preads_fasta_fn, 'r')  as f:
        for each in f:
            each = each.strip()
            if each[0] == '>':
                rn = each[1:]
    ...
    edge_data = {}
    # edges.append((vv, ww, rid, ss, tt, aln_score, idt, "G"))
    for v, w in edges.keys():
        ...
        #print(v, w,rid, ss, tt, aln_score, idt)
    ...
    for l in ctg.keys():
        ...
        #ctg_id, c_type_, i_utig, t0, length, score, utgs = ctg[l]
...
"""
    ...
    parser.add_argument('--paf_fn', type=str,
                        help='Input. Preads alignment file, used to construct string graph edges. Optional.')
