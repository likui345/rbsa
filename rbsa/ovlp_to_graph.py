...
def rm_lowcoverage(sg):
    sg1 = StringGraph()
    for v, w in sg.e_reduce: # sort, or OrderedDict
        e = sg.edges[(v, w)]
        label = e.attr["label"]
        score = e.attr["score"]
        identity = e.attr["identity"]
        length = e.attr["length"]
        sg1.add_edge(v, w, label=label, length=length, score=score, identity = identity )
        del sg.edges[(v,w)]
    sg1.init_reduce_dict()
    sg1.mark_tr_edges()  # mark those edges that transitive redundant
    return sg1



def init_digraph(sg, chimer_edges, removed_edges, spur_edges):
    with open("sg_edges_list1", "w") as out_f:
...
def yield_from_overlap_file(overlap_file,scores = -500):
...
def ovlp_to_graph(args):
    ...
    generate_nx_string_graph(sg1, args.lfc, args.disable_chimer_bridge_removal)
